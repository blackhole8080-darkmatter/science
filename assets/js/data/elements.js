/**
 * Periodic table dataset: 118 elements.
 * Row format: [Z, symbol, name, standard atomic weight, category, group, period]
 * A group of 0 marks an f-block element (lanthanide / actinide) that sits in the
 * detached rows below the main table.
 */

const RAW = [
  [1, "H", "Hydrogen", 1.008, "nonmetal", 1, 1],
  [2, "He", "Helium", 4.0026, "noble", 18, 1],
  [3, "Li", "Lithium", 6.94, "alkali", 1, 2],
  [4, "Be", "Beryllium", 9.0122, "alkaline", 2, 2],
  [5, "B", "Boron", 10.81, "metalloid", 13, 2],
  [6, "C", "Carbon", 12.011, "nonmetal", 14, 2],
  [7, "N", "Nitrogen", 14.007, "nonmetal", 15, 2],
  [8, "O", "Oxygen", 15.999, "nonmetal", 16, 2],
  [9, "F", "Fluorine", 18.998, "halogen", 17, 2],
  [10, "Ne", "Neon", 20.180, "noble", 18, 2],
  [11, "Na", "Sodium", 22.990, "alkali", 1, 3],
  [12, "Mg", "Magnesium", 24.305, "alkaline", 2, 3],
  [13, "Al", "Aluminium", 26.982, "post", 13, 3],
  [14, "Si", "Silicon", 28.085, "metalloid", 14, 3],
  [15, "P", "Phosphorus", 30.974, "nonmetal", 15, 3],
  [16, "S", "Sulfur", 32.06, "nonmetal", 16, 3],
  [17, "Cl", "Chlorine", 35.45, "halogen", 17, 3],
  [18, "Ar", "Argon", 39.95, "noble", 18, 3],
  [19, "K", "Potassium", 39.098, "alkali", 1, 4],
  [20, "Ca", "Calcium", 40.078, "alkaline", 2, 4],
  [21, "Sc", "Scandium", 44.956, "transition", 3, 4],
  [22, "Ti", "Titanium", 47.867, "transition", 4, 4],
  [23, "V", "Vanadium", 50.942, "transition", 5, 4],
  [24, "Cr", "Chromium", 51.996, "transition", 6, 4],
  [25, "Mn", "Manganese", 54.938, "transition", 7, 4],
  [26, "Fe", "Iron", 55.845, "transition", 8, 4],
  [27, "Co", "Cobalt", 58.933, "transition", 9, 4],
  [28, "Ni", "Nickel", 58.693, "transition", 10, 4],
  [29, "Cu", "Copper", 63.546, "transition", 11, 4],
  [30, "Zn", "Zinc", 65.38, "transition", 12, 4],
  [31, "Ga", "Gallium", 69.723, "post", 13, 4],
  [32, "Ge", "Germanium", 72.630, "metalloid", 14, 4],
  [33, "As", "Arsenic", 74.922, "metalloid", 15, 4],
  [34, "Se", "Selenium", 78.971, "nonmetal", 16, 4],
  [35, "Br", "Bromine", 79.904, "halogen", 17, 4],
  [36, "Kr", "Krypton", 83.798, "noble", 18, 4],
  [37, "Rb", "Rubidium", 85.468, "alkali", 1, 5],
  [38, "Sr", "Strontium", 87.62, "alkaline", 2, 5],
  [39, "Y", "Yttrium", 88.906, "transition", 3, 5],
  [40, "Zr", "Zirconium", 91.224, "transition", 4, 5],
  [41, "Nb", "Niobium", 92.906, "transition", 5, 5],
  [42, "Mo", "Molybdenum", 95.95, "transition", 6, 5],
  [43, "Tc", "Technetium", 98, "transition", 7, 5],
  [44, "Ru", "Ruthenium", 101.07, "transition", 8, 5],
  [45, "Rh", "Rhodium", 102.91, "transition", 9, 5],
  [46, "Pd", "Palladium", 106.42, "transition", 10, 5],
  [47, "Ag", "Silver", 107.87, "transition", 11, 5],
  [48, "Cd", "Cadmium", 112.41, "transition", 12, 5],
  [49, "In", "Indium", 114.82, "post", 13, 5],
  [50, "Sn", "Tin", 118.71, "post", 14, 5],
  [51, "Sb", "Antimony", 121.76, "metalloid", 15, 5],
  [52, "Te", "Tellurium", 127.60, "metalloid", 16, 5],
  [53, "I", "Iodine", 126.90, "halogen", 17, 5],
  [54, "Xe", "Xenon", 131.29, "noble", 18, 5],
  [55, "Cs", "Caesium", 132.91, "alkali", 1, 6],
  [56, "Ba", "Barium", 137.33, "alkaline", 2, 6],
  [57, "La", "Lanthanum", 138.91, "lanthanide", 3, 6],
  [58, "Ce", "Cerium", 140.12, "lanthanide", 0, 6],
  [59, "Pr", "Praseodymium", 140.91, "lanthanide", 0, 6],
  [60, "Nd", "Neodymium", 144.24, "lanthanide", 0, 6],
  [61, "Pm", "Promethium", 145, "lanthanide", 0, 6],
  [62, "Sm", "Samarium", 150.36, "lanthanide", 0, 6],
  [63, "Eu", "Europium", 151.96, "lanthanide", 0, 6],
  [64, "Gd", "Gadolinium", 157.25, "lanthanide", 0, 6],
  [65, "Tb", "Terbium", 158.93, "lanthanide", 0, 6],
  [66, "Dy", "Dysprosium", 162.50, "lanthanide", 0, 6],
  [67, "Ho", "Holmium", 164.93, "lanthanide", 0, 6],
  [68, "Er", "Erbium", 167.26, "lanthanide", 0, 6],
  [69, "Tm", "Thulium", 168.93, "lanthanide", 0, 6],
  [70, "Yb", "Ytterbium", 173.05, "lanthanide", 0, 6],
  [71, "Lu", "Lutetium", 174.97, "lanthanide", 0, 6],
  [72, "Hf", "Hafnium", 178.49, "transition", 4, 6],
  [73, "Ta", "Tantalum", 180.95, "transition", 5, 6],
  [74, "W", "Tungsten", 183.84, "transition", 6, 6],
  [75, "Re", "Rhenium", 186.21, "transition", 7, 6],
  [76, "Os", "Osmium", 190.23, "transition", 8, 6],
  [77, "Ir", "Iridium", 192.22, "transition", 9, 6],
  [78, "Pt", "Platinum", 195.08, "transition", 10, 6],
  [79, "Au", "Gold", 196.97, "transition", 11, 6],
  [80, "Hg", "Mercury", 200.59, "transition", 12, 6],
  [81, "Tl", "Thallium", 204.38, "post", 13, 6],
  [82, "Pb", "Lead", 207.2, "post", 14, 6],
  [83, "Bi", "Bismuth", 208.98, "post", 15, 6],
  [84, "Po", "Polonium", 209, "metalloid", 16, 6],
  [85, "At", "Astatine", 210, "halogen", 17, 6],
  [86, "Rn", "Radon", 222, "noble", 18, 6],
  [87, "Fr", "Francium", 223, "alkali", 1, 7],
  [88, "Ra", "Radium", 226, "alkaline", 2, 7],
  [89, "Ac", "Actinium", 227, "actinide", 3, 7],
  [90, "Th", "Thorium", 232.04, "actinide", 0, 7],
  [91, "Pa", "Protactinium", 231.04, "actinide", 0, 7],
  [92, "U", "Uranium", 238.03, "actinide", 0, 7],
  [93, "Np", "Neptunium", 237, "actinide", 0, 7],
  [94, "Pu", "Plutonium", 244, "actinide", 0, 7],
  [95, "Am", "Americium", 243, "actinide", 0, 7],
  [96, "Cm", "Curium", 247, "actinide", 0, 7],
  [97, "Bk", "Berkelium", 247, "actinide", 0, 7],
  [98, "Cf", "Californium", 251, "actinide", 0, 7],
  [99, "Es", "Einsteinium", 252, "actinide", 0, 7],
  [100, "Fm", "Fermium", 257, "actinide", 0, 7],
  [101, "Md", "Mendelevium", 258, "actinide", 0, 7],
  [102, "No", "Nobelium", 259, "actinide", 0, 7],
  [103, "Lr", "Lawrencium", 266, "actinide", 0, 7],
  [104, "Rf", "Rutherfordium", 267, "transition", 4, 7],
  [105, "Db", "Dubnium", 268, "transition", 5, 7],
  [106, "Sg", "Seaborgium", 269, "transition", 6, 7],
  [107, "Bh", "Bohrium", 270, "transition", 7, 7],
  [108, "Hs", "Hassium", 269, "transition", 8, 7],
  [109, "Mt", "Meitnerium", 278, "transition", 9, 7],
  [110, "Ds", "Darmstadtium", 281, "transition", 10, 7],
  [111, "Rg", "Roentgenium", 282, "transition", 11, 7],
  [112, "Cn", "Copernicium", 285, "transition", 12, 7],
  [113, "Nh", "Nihonium", 286, "post", 13, 7],
  [114, "Fl", "Flerovium", 289, "post", 14, 7],
  [115, "Mc", "Moscovium", 290, "post", 15, 7],
  [116, "Lv", "Livermorium", 293, "post", 16, 7],
  [117, "Ts", "Tennessine", 294, "halogen", 17, 7],
  [118, "Og", "Oganesson", 294, "noble", 18, 7],];

/**
 * Category colours, ordered to follow the table's own left-to-right adjacency
 * (alkali → alkaline → transition → … → noble). That ordering was solved for
 * maximum separation between neighbouring blocks and validated for colour-vision
 * deficiency: worst adjacent pair ΔE 13.2 (CVD) / 19.3 (normal vision) on both
 * the dark and light surfaces. Non-neighbouring blocks can look closer, so every
 * cell also carries its symbol and atomic number, and the legend and element
 * table repeat the category in text — colour is never the only cue.
 */
export const CATEGORIES = {
  alkali: { label: "Alkali metal", color: "#3f8f2f" },
  alkaline: { label: "Alkaline earth metal", color: "#b06ab0" },
  transition: { label: "Transition metal", color: "#c98500" },
  post: { label: "Post-transition metal", color: "#d55181" },
  metalloid: { label: "Metalloid", color: "#9085e9" },
  nonmetal: { label: "Reactive nonmetal", color: "#199e70" },
  halogen: { label: "Halogen", color: "#3987e5" },
  noble: { label: "Noble gas", color: "#d95926" },
  lanthanide: { label: "Lanthanide", color: "#22a5b8" },
  actinide: { label: "Actinide", color: "#e66767" },
};

/** Orbital fill order (Madelung / aufbau rule). */
const ORBITALS = [
  ["1s", 2], ["2s", 2], ["2p", 6], ["3s", 2], ["3p", 6], ["4s", 2], ["3d", 10],
  ["4p", 6], ["5s", 2], ["4d", 10], ["5p", 6], ["6s", 2], ["4f", 14], ["5d", 10],
  ["6p", 6], ["7s", 2], ["5f", 14], ["6d", 10], ["7p", 6],
];

/**
 * Predicted ground-state configuration from the aufbau order. A handful of
 * elements (Cr, Cu, Pd, ...) deviate experimentally; callers label it as predicted.
 */
export function electronConfiguration(z) {
  let left = z;
  const parts = [];
  for (const [name, capacity] of ORBITALS) {
    if (left <= 0) break;
    const n = Math.min(left, capacity);
    parts.push(name + n);
    left -= n;
  }
  return parts.join(" ");
}

/** Electrons per shell, e.g. 11 -> [2, 8, 1]. */
export function shellOccupancy(z) {
  const shells = [];
  let left = z;
  for (const [name, capacity] of ORBITALS) {
    if (left <= 0) break;
    const shell = Number(name[0]) - 1;
    const n = Math.min(left, capacity);
    shells[shell] = (shells[shell] || 0) + n;
    left -= n;
  }
  return shells.map((n) => n || 0);
}

export const ELEMENTS = RAW.map(([number, symbol, name, mass, category, group, period]) => ({
  number,
  symbol,
  name,
  mass,
  category,
  group: group || null,
  period,
  block: group === 0 ? "f" : group === 1 || group === 2 ? "s" : group >= 13 ? "p" : "d",
  color: CATEGORIES[category].color,
}));

export const BY_SYMBOL = new Map(ELEMENTS.map((e) => [e.symbol, e]));
export const BY_NUMBER = new Map(ELEMENTS.map((e) => [e.number, e]));

/**
 * Grid position for the standard 18-column layout. The lanthanides and
 * actinides — all fifteen of each, lanthanum and actinium included — are laid
 * out in two detached rows (8 and 9), leaving column 3 of periods 6 and 7 free
 * for the "57–71" and "89–103" placeholder cells.
 */
export function gridPosition(element) {
  if (element.category === "lanthanide" || element.category === "actinide") {
    const anchor = element.category === "lanthanide" ? 57 : 89;
    return { row: element.category === "lanthanide" ? 8 : 9, col: 4 + (element.number - anchor) };
  }
  return { row: element.period, col: element.group };
}
